...
xxxx <- cargo_tanker_summary%>%
  ...
  select(name, mmsi, company, num_trips, avg_time, max_time, min_time, max_speed)

yy <- xxxx %>%
  select("Vessel Name" =name, "Vessel MMSI"=mmsi, "Vessel Company"= company, "Number of Transits"=num_trips, "Average Transit Time (hrs)"=avg_time, "Max Transit Time (hrs)"=max_time, "Min Transit Time (hrs)"=min_time, "Max speed observed (kn)" =max_speed)
...
one_company_x <- subset(one_company, company %in% company_1)
